...
export default function Form({ isHidden, onExit }: Props){
    return(
        <>
            <div onClick={onExit} className={`${isHidden ? "hidden" : "visible"} fixed w-full h-full top-0 left-0 backdrop-blur-sm z-30`}>
            </div>
            <div className={`${isHidden ? "hidden" : "visible"} fixed w-[60%] h-[700px] left-96 top-40 p-2 rounded-xl info shadow-2xl border-1 border-black bg-white z-50`}>
